...
- (BOOL)initializeDB
{
    ...
    if ([ [GDFileManager defaultManager] fileExistsAtPath:[self documentsFolderPathForFileNamed:self.databaseFilename]]==YES)
        NSLog(@"DEBUG: Database File exists.");
    
    // open databse file
    int returnCode = sqlite3enc_open([[self documentsFolderPathForFileNamed:self.databaseFilename]UTF8String], &sqlite3Database);
...
-(void)runQuery:(const char *)query isQueryExecutable:(BOOL)queryExecutable{
    ...
    //Check if the database already exists
    if ([ [GDFileManager defaultManager] fileExistsAtPath:databasePath]==YES)
        NSLog(@"DEBUG: Database File exists.");
        ...
        if(prepareStatementResult == SQLITE_OK)
        {
            ...
            if (queryExecutable)
            {
                ...
                else
                {
                    ...
                    NSLog(@"DEBUG: DB Error: %s", sqlite3_errmsg(sqlite3Database));
                }
            }else {
