...
open class Service: NSObject, GDServiceClientDelegate, GDServiceDelegate {
    ...
    func sendBringToFront() throws {
        try GDServiceClient.bring(toFront: Constants.servicesClientGDIdentifier, completion: { (success) in
    ...
    func processRequestFor(_ application: String!, forService service: String!, withVersion version: String!, forMethod method: String!, withParams params: AnyObject!, withAttachments attachments: [AnyObject]!, forRequestID requestID: String!) {
        ...
        if !consumeFrontRequestService(service, forApplication: application, forMethod: method, withVersion: version) {
            if service == Constants.greetingsGDServiceIdentifier {
                if version == Constants.greetingsGDServiceVersionIdentifier {
                    if method == Constants.greetingsGDServiceMethodIdentifier {
                        try! GDService.reply(to: application, withParams: "G'day mate!", bringClientToFront: .GDEPreferPeerInForeground, withAttachments: nil, requestID: requestID)
                    }
                }
            } else if service == Constants.dateAndTimeGDServiceIdentifier {
                if version == Constants.dateAndTimeGDServiceVersionIdentifier {
                    if method == Constants.dateAndTimeGDServiceMethodIdentifier {
                        let date = Date.init()
                        let formatter = DateFormatter()
                        formatter.dateFormat = "MM/dd/yyyy hh:mma"
                        let string = formatter.string(from: date)
                        try! GDService.reply(to: application, withParams: string, bringClientToFront: .GDEPreferPeerInForeground, withAttachments: nil, requestID: requestID)
                    }
                }
            } else if service == Constants.fileTransferGDServiceIdentifier {
        ...
        if service == GDFrontRequestService && version == "1.0.0" {
            if method == GDFrontRequestMethod {
                try! GDService.bring(toFront: application, completion: { (success) in
                    print("bring to front \(application) completed")
                })
            } else {
