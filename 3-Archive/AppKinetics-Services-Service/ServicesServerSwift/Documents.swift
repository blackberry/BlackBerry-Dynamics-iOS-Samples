...
open class Documents {
    ...
    // Update
    func rename(_ fromFileName: String, toFileName: String, index: Int) {
        if manager.fileExists(atPath: Documents.directory + fromFileName) && !manager.fileExists(atPath: Documents.directory + toFileName) {
            try! manager.moveItem(atPath: Documents.directory + fromFileName, toPath: Documents.directory + toFileName)
            print("moved")
            list[index] = toFileName
        }
        if manager.fileExists(atPath: Documents.directory + fromFileName) && !manager.fileExists(atPath: Documents.directory + toFileName) {
            print("error")
        }
