...
static NSString* const RSSFeedName = @"Test feeld";
static NSString* const RSSFeedURL  = @"http://rss.cnn.com/rss/edition_world.rss";
...
- (void)testProvision
{
    ...
    BOOL mainUIShown = [ats isScreenShown:RSSFeedListViewID timeout:10.f];
    
    if (!mainUIShown && UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        [application swipeRight];
        mainUIShown = [ats isScreenShown:RSSFeedListViewID timeout:10.f];
    }
