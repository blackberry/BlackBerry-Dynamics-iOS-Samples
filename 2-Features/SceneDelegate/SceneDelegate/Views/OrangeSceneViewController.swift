...
class OrangeSceneViewController: UIViewController {
    
    // MARK: IBOutlets
    @IBOutlet weak var numberOfOrangeScenesLabel: UILabel!
    ...
    override func loadView() {
        super.loadView()
        self.presenter = OrangeViewControllerPresenter(view: self)
    }
...
extension OrangeSceneViewController: ColorView {
    
    func updateViewsCount(_ count: Int, with color: UIColor) {
        self.numberOfOrangeScenesLabel.text = String(count)
        self.numberOfOrangeScenesLabel.textColor = color
