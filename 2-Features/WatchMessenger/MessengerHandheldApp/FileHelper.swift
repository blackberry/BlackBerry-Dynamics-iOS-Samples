...
class FileHelper {
    ...
    func initialiseMessageFile() {
        ...
        let now = "Phone \(dateFormatter.string(from: Date.now))"
    ...
    func createFileInWatchFolder() {
        let fileName = "PF-\(UUID()).txt"
        let fileContent = "\"\(fileName)\" created on \(dateFormatter.string(from: Date.now))"
        let fileData = fileContent.data(using: .utf8)
        gdFileManager.createFile(atPath: watchFilesFolder + fileName, contents: fileData)
    }
    ...
    func resetFolderFilesFromWatch() {
