...
class MessengerHandheldAppGDiOSDelegate: NSObject, GDiOSDelegate {
    
    static let sharedInstance = MessengerHandheldAppGDiOSDelegate()
    weak var appDelegate: AppDelegate?
    var hasAuthorized = false
    
    var isWatchAllowed: Bool {
        get {
            return GDiOS.sharedInstance().getApplicationConfig()[GDAppConfigKeyAllowWearable] as? Int == 1
        }
    }
    ...
    func didAuthorize() -> Void {
        if (hasAuthorized && (appDelegate != nil) ) {
